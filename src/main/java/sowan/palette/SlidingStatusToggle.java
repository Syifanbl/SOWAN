...
public class SlidingStatusToggle extends JToggleButton {
    ...
    // Variabel untuk menyimpan teks agar tidak dipanggil berulang kali di paintComponent
    private String textIn = "Masuk"; 
    private String textOut = "Pulang";
    ...
    public SlidingStatusToggle() {
        ...
        // PENCEGAHAN CRASH DI NETBEANS: 
        // Hanya panggil bahasaService jika aplikasi sedang di-Run (bukan saat diedit di GUI Builder)
        if (!Beans.isDesignTime()) {
            try {
                textIn = bahasaService.get("ui.status.in");
                textOut = bahasaService.get("ui.status.out");
            } catch (Exception e) {
                // Fallback aman jika service bermasalah
                textIn = "Masuk";
                textOut = "Pulang";
            }
        }
    ...
    public void setStatusByString(String status) {
        if (!Beans.isDesignTime() && bahasaService.get("ui.status.out").equalsIgnoreCase(status)) {
    ...
    public String getStatusString() {
        if (Beans.isDesignTime()) {
            return isSelected() ? textOut : textIn;
        }
        return isSelected() ? bahasaService.get("ui.status.out") : bahasaService.get("ui.status.in");  
    }
    ...
    protected void paintComponent(Graphics g) {
        ...
        // Teks Sisi Kiri -> MASUK (Menggunakan variabel yang sudah di-load di constructor)
        int textLeftX = (w / 4) - (fm.stringWidth(textIn) / 2); // Presisi tengah di area kiri
        ...
        g2.drawString(textIn, textLeftX, textY);

        // Teks Sisi Kanan -> PULANG (Menggunakan variabel yang sudah di-load di constructor)
        int textRightX = ((w / 4) * 3) - (fm.stringWidth(textOut) / 2); // Presisi tengah di area kanan

        // Jika aktif di kanan, teks berwarna putih terang. Jika tidak, abu-abu redup.
        g2.setColor(isPulangActive ? Color.WHITE : new Color(130, 135, 145));
        g2.drawString(textOut, textRightX, textY);
